define(['app'], function(app) {
    app.controller('editePluginunitController', editePluginunitController);
    editePluginunitController.$inject = ['$scope', '$http', '$location', '$state', 'releasePluginunitService', 'baseUrl', 'FileUploader','$cookies'];

    function editePluginunitController($scope, $http, $location, $state, releasePluginunitService, baseUrl, FileUploader,$cookies) {
        ...
        services.getLabels(1).then(function(res) {
            if (res.status === 200) {
                ...
                // console.log($scope.labels_no)
                // console.log($scope.labels_have)
            }

        });
        // 获取插件详情
        var id = $location.search().pluginunitId;
        services.getPlugins(id).then(function(res) {
            console.log(res.data.data)
            if (res.data.code === 1000) {
                $scope.data = res.data.data;
                if ($scope.data.servicebox_url) {
...
// hyf
//                     for (var i = 0, len = $scope.data.labels.length; i < len; i++) {
//                         $scope.data.label_ids.push($scope.data.labels[i]);
//                         $scope.selectedLabel_have.push($scope.data.labels[i].title);
//                         $scope.labelsObj_have = $scope.data.labels;
//hyf
// tgd
...
//tgd
                    ...
                    console.log($scope.data1.image_urls)
                    if ($scope.data1.image_urls) {
        ...
        $scope.save = function() {
            ...
            if ($scope.currentOrder === 'noService') {
                ...
                delete $scope.data1.servicebox_url;
                console.log('data1')
                ...
                if (optionsObj_data3_pluginsbox.who1) {
                    ...
                    $scope.data3.pluginsbox_url = $scope.data2.pluginsbox_url;
                }
                if($cookies.get("user_id")){
